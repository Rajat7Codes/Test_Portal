 ...
 */
@Controller
public class DriveStartTestController {
	public DriveStartTestController() {
	...
	/* TEST LIST PAGE */
	@RequestMapping("/drive/student/test/list")
	public String testList(ModelMap modelMap, Locale locale) throws ResourceNotFoundException, ParseException {
		...
		for (AddTest addTest : this.addTestService.getAddTestList()) {
			for (TestResult result : testResultService.getTestResultList()) {
				if (this.userService.findBySSO(getPrincipal()).getId() == result.getUserId()) {
					if (addTest.getDepartmentName().equals("DRIVE")) {
		...
		if (!testList.isEmpty()) {
			...
			for (AddTest test : addTestService.getAddTestList()) {
				if (test.getDepartmentName().equals("DRIVE")) {
					addTestList.add(test);
				}
			}
		}
	...
	/* START TEST PAGE */
	@RequestMapping("/drive/student/start/test/{testId}")
	...
	@RequestMapping(value = "/drive/student/start/test/compiler", produces = MediaType.APPLICATION_JSON_UTF8_VALUE, method = RequestMethod.GET)
	...
	@RequestMapping(value = "/drive/student/end/test", produces = MediaType.APPLICATION_JSON_UTF8_VALUE, method = RequestMethod.GET)
